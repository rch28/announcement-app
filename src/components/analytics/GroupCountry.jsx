...
const GroupImpressionCountry = ({ group_id }) => {
    ...
    const fetchData = async () => {
      ...
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_DB_BASE_URL}/analytics/group/${group_id}/impression/country-city/?type=${selectedFilter}`,
          {
            method: "GET",
            headers: {
  ...
  return (
    <div className="w-[500px] h-fit shadow-custom-all p-10 rounded-xl">
      ...
      {loading ? (
        ...
        <div className="flex justify-center items-center h-64">
          <Link
            href={`/payment/checkout?group_id=${group_id}`}
